...
- **Ativar IA real (já com memória):** crie um `.env.local` na raiz com:
  ...
  ```
  Depois rode `npm run dev` ou `npm run build && npm run preview`. O coach usa esses valores automaticamente; se não definir, ele roda em modo demonstração. Reinicie o servidor se alterar o `.env.local`.
